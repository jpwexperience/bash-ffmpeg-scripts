...
if [ "$#" -eq 0 ]; then
  ...
  echo "For Option List use: $(basename $0) -h"
  exit 0
fi

while getopts i:o:s:t:v:c:w:e:l:z:ahg opt; do
  case $opt in
    i)
      fileIn="$OPTARG"
      ;;
    ...
    l)
      ...
      ;;
    z)
      size="$OPTARG" #fps of gif
      ;;
    a)
      audio="an" #remove audio
      ;;
    h)
      HELP="1" #list options
      ;;
...
if [ ! -z "$HELP" ]; then
	...
	echo -e "-z bitrate\t\tBitrate of resulting webM in MB [default: 0.35MB]"
	exit 0
fi
...
if [[ "$subType" == "n" ]]; then
	if [[ "$cropW" == "-1" ]]; then
		if [[ "$audio" == "an" ]]; then
			if [[ "$scale" == -1 ]]; then
				cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffCv -an $ffCrf $ffBv $ffOut"
			else
				cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffScale $ffCv -an $ffCrf $ffBv $ffOut"
			fi
		else
			if [[ "$scale" == -1 ]]; then
				cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffAmap $ffCv $ffCa $ffCrf $ffBv $ffOut"
			else
				cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffScale $ffAmap $ffCv $ffCa $ffCrf $ffBv $ffOut"
			fi
		fi
	else
		if [[ "$audio" == "an" ]]; then
			cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffCropScale $ffCv -an $ffCrf $ffBv $ffOut"
		else
			cutCmd="$ffBeg $ffStart $ffIn $ffDur $ffVmap $ffCropScale $ffCv $ffAmap $ffCa $ffCrf $ffBv $ffOut"
		fi
	fi
else
        if [[ "$subType" == "i" ]]; then
			...
			if [[ "$audio" == "an" ]]; then
				cutCmd="$cutCmd -an $ffCrf $ffBv $ffOut"
			else
				cutCmd="$cutCmd $ffAmap $ffCa $ffCrf $ffBv $ffOut"
			fi
                else
			...
			if [[ "$audio" == "an" ]]; then
                ...
                cutCmd="$ffBeg $ffIn $ffVmap $ffStart $ffDur $subtitleCmd"
		if [[ "$audio" == "an" ]]; then
			cutCmd="$cutCmd $ffAmap $ffCv -an $ffCrf $ffBv $ffOut"
		else
			cutCmd="$cutCmd $ffAmap $ffCv $ffCa $ffCrf $ffBv $ffOut"
		fi
        fi
fi
